...
class TestBankTransferPaymentCreation(BuckarooOfficialCommon):
    ...
    def _sdk_response(self, redirect_url="https://testcheckout.buckaroo.nl/pay/BT", key="BT_KEY"):
        response = MagicMock()
    ...
    def test_payment_creation_returns_redirect_and_stores_reference(self):
        ...
        with (
            ...
            patch.object(
                PaymentMethod, "_buckaroo_extract_redirect_url", return_value="https://bt/pay/X"
            ),
        ):
...
class TestBankTransferDetailCapture(BuckarooOfficialCommon):
    ...
    def _make_sdk_response_with_services(self):
        ...
        response.get_service_parameter.side_effect = lambda name: next(
