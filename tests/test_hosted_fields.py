...
class TestTransactionHostedFields(BuckarooOfficialCommon):
    ...
    def test_processing_uses_redirect_when_no_session_in_http_session(self):
        ...
        with patch(
            ...
            type(tx), '_buckaroo_official_get_client', return_value=MagicMock()
        ), patch.object(
            type(tx.payment_method_id), '_buckaroo_official_is_amount_compatible', return_value=True
        ):
...
class TestHostedFieldsTokenEndpoint(BuckarooOfficialCommon):
    ...
    def test_endpoint_route_exists(self):
        ...
        self.assertIsNotNone(method, "hosted_fields_token method must exist on CreditCardController")
    ...
    def test_token_exchange_success(self):
        ...
        with patch.object(
            type(self.env['payment.provider']), 'sudo',
...
class TestCreditCardBrandMethods(BuckarooOfficialCommon):
    ...
    def test_expected_brand_codes(self):
        ...
        expected = {
            'visa', 'mastercard', 'amex', 'maestro', 'visaelectron',
            'vpay', 'cartebancaire', 'cartebleuevisa', 'dankort', 'nexi',
            'postepay',
        }
