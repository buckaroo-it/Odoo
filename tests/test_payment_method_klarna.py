...
def _set_session_gender(value="1"):
    ...
    return patch(
        "odoo.addons.payment_buckaroo_official.models.payment_method_klarna.http_request"
        if False
        else "odoo.http.request",
        fake_request,
    )
...
class TestKlarnaPaymentCreation(BuckarooOfficialCommon):
    ...
    def _sdk_response(self, redirect_url="https://testcheckout.buckaroo.nl/pay/KL", key="KL_KEY"):
        response = MagicMock()
    ...
    def test_payment_creation_returns_redirect_and_stores_reference(self):
        ...
        with (
            ...
            patch.object(
                PaymentMethod, "_buckaroo_extract_redirect_url", return_value="https://kl/pay/X"
            ),
        ):
...
class TestKlarnaSendCaptureVoidRequest(BuckarooOfficialCommon):
    ...
    def test_void_via_full_dispatch_calls_sdk_cancel_reservation(self):
        ...
        tx = self._create_authorized_tx(
            reference="KL-FULL-VOID", provider_reference="RES_FULL_VOID"
        )
        mock_builder, mock_response = make_mock_sdk_builder()
...
class TestKlarnaIdempotentResponseNormalization(BaseCase):
    ...
    def test_already_captured_490_normalizes_to_190(self):
        msg = (
            "An error occurred while processing the transaction: BadRequest: "
    ...
    def test_already_cancelled_490_normalizes_to_190(self):
        msg = (
            "Error OrderService_Cancel_InvalidOrderStatus: Order with Id X "
