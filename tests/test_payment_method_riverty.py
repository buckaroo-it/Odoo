...
class TestRivertyPaymentCreation(BuckarooOfficialCommon):
    ...
    def _sdk_response(self, redirect_url='https://testcheckout.buckaroo.nl/pay/RV',
                      key='RV_KEY'):
        response = MagicMock()
    ...
    def test_refund_status_matrix(self):
        for status_code, expected_state in REFUND_STATUS_CASES:
            with self.subTest(status_code=status_code, expected_state=expected_state):
                ...
                with self._patch_refund(self._mock_refund_response(
                    key='RV_REFUND_%s' % status_code, status_code=status_code,
                )):
                    refund_tx = tx._refund()
...
class TestRivertyCurrencyAndCountryRestriction(BuckarooOfficialCommon):
    ...
    def setUpClass(cls):
        ...
        cls.partner_us = cls.env['res.partner'].create({
            'name': 'US', 'country_id': cls.env.ref('base.us').id,
        })
...
class TestRivertyImageUrl(BuckarooOfficialCommon):
    ...
    def _format(self, article, base_url='https://shop.test'):
        return RivertyPaymentMethod._format_riverty_articles(
            [{
                'identifier': 'X', 'description': 'X', 'quantity': 1.0,
                'unit_price_incl': 10.0, 'unit_price_excl': 10.0,
                'vat_percentage': 0.0, 'vat_amount': 0.0,
                **article,
            }],
            base_url,
        )[0]
...
class TestFormatRivertyArticles(BaseCase):
    ...
    def test_shipping_line_maps_to_shipping_fee_type(self):
        generic = [{
            'identifier': 'SHIP', 'description': 'Shipping', 'quantity': 1.0,
            'unit_price_incl': 5.0, 'unit_price_excl': 5.0,
            'vat_percentage': 0.0, 'vat_amount': 0.0, 'type': 'shipping',
        }]
        result = RivertyPaymentMethod._format_riverty_articles(generic)
    ...
    def test_rounding_line_maps_to_surcharge_type(self):
        ...
        generic = [{
            'identifier': 'rounding', 'description': 'Rounding correction',
            'quantity': 1.0, 'unit_price_incl': 0.01, 'unit_price_excl': 0.01,
            'vat_percentage': 0.0, 'vat_amount': 0.0, 'type': 'rounding',
        }]
        result = RivertyPaymentMethod._format_riverty_articles(generic)
    ...
    def test_fractional_quantity_rounds_to_nearest_int(self):
        ...
        generic = [{
            'identifier': 'KG', 'description': 'Cheese', 'quantity': 1.5,
            'unit_price_incl': 10.0, 'unit_price_excl': 10.0,
    ...
    def test_sub_unit_quantity_clamps_to_one(self):
        ...
        generic = [{
            'identifier': 'SMALL', 'description': 'Small Sample',
            'quantity': 0.4, 'unit_price_incl': 10.0, 'unit_price_excl': 10.0,
            'vat_percentage': 0.0, 'vat_amount': 0.0, 'type': 'product',
        }]
        result = RivertyPaymentMethod._format_riverty_articles(generic)
    ...
    def test_image_url_omitted_when_no_product_reference(self):
        generic = [{
            'identifier': 'SKU-NOIMG', 'description': 'Widget', 'quantity': 1.0,
            'unit_price_incl': 10.0, 'unit_price_excl': 10.0,
            'vat_percentage': 0.0, 'vat_amount': 0.0, 'type': 'product',
            'product': None,
        }]
