...
class TestBuckarooOfficialProcessingFlows(BuckarooOfficialCommon):
    ...
    def _make_sdk_response(
        self,
        redirect_url="https://testcheckout.buckaroo.nl/pay/123",
        key="BUCK_TXN_KEY_123",
    ...
    def test_refund_creates_child_transaction(self):
        ...
        with patch.object(
            PaymentMethod, "_buckaroo_create_refund", return_value=self._mock_refund_response()
        ):
...
class TestBuckarooOfficialE2ESandbox(BuckarooOfficialCommon):
    ...
    def _create_done_tx_for_method(self, method_code):
        ...
        pm_record = self.env["payment.method"].search([("code", "=", method_code)], limit=1)
        ...
        tx._apply_updates(
            self._get_buckaroo_callback_data(
                status_code=190,
    ...
    def _assert_create_refund_returns_valid_response(self, method_code):
        ...
        self.assertTrue(
            refund_tx.exists(), "Refund transaction must be created for %s" % method_code
        )
        self.assertEqual(refund_tx.operation, "refund")
