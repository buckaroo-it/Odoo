...
def _make_mock_sdk_response(redirect_url='https://checkout.buckaroo.nl/pay/123',
                            key='TXN_KEY_123', status_code=190):
...
class TestTransactionModelDispatch(BuckarooOfficialCommon):
    ...
    def test_processing_values_calls_model_create_payment(self):
        ...
        with patch.object(
            type(self.ideal), '_buckaroo_create_payment',
            return_value=mock_response,
        ) as mock_create, patch.object(
            type(self.ideal), '_buckaroo_extract_redirect_url',
    ...
    def test_refund_calls_model_create_refund(self):
        ...
        with patch.object(
            type(self.ideal), '_buckaroo_create_refund',
            return_value=mock_response,
        ) as mock_refund:
    ...
    def test_capture_calls_model_create_capture(self):
        ...
        with patch.object(
            type(creditcard), '_buckaroo_create_capture',
            return_value=mock_response,
        ) as mock_capture:
    ...
    def test_void_calls_model_create_void(self):
        ...
        with patch.object(
            type(creditcard), '_buckaroo_create_void',
            return_value=mock_response,
        ) as mock_void:
            void_tx = tx._void()
