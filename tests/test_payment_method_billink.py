...
class TestBillinkPaymentCreation(BuckarooOfficialCommon):
    ...
    def _sdk_response(self, redirect_url="https://testcheckout.buckaroo.nl/pay/BL", key="BL_KEY"):
        response = MagicMock()
    ...
    def test_payment_creation_returns_redirect_and_stores_reference(self):
        ...
        with (
            ...
            patch.object(
                PaymentMethod, "_buckaroo_extract_redirect_url", return_value="https://bl/pay/X"
            ),
        ):
...
class TestBillinkOrderLineFormatting(BaseCase):
    def _make_tx(self, lines, amount=None):
        total = (
            ...
            else sum(l.price_total for l in lines if not l.display_type)
        )
        tx = MagicMock()
        tx.amount = total
        order = MagicMock()
